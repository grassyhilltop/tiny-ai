...
6. **Graded like a game:** held-out test set (base64-encoded in `testset.b64` so it is not read while the filter is being written — honor system), 5 stars, over-blocking penalized as much as under-blocking. Good solutions discover allowlist + blocklist thinking on their own: the fruit words to catch, plus the exceptions to let through ("Apple Inc. earnings", "the Grapefruit League", "kiwi the bird").
...
10. **UVA-specific material removed** for the public fork — schedules, HPC/Rivanna paths, and the rest of the original course; only the two general machine-learning labs are adapted here.
11. Title direction: "LLM Foundations for All" (keeps Joel's "for all" signal). a nod to the Hack Box prototyping kit from his 2016 thesis can land in Lab 1 copy later. Forbidden Fruit is the alignment game's name.
...
- Repo: `github.com/grassyhilltop/tiny-ai` — public, Pages enabled, custom domain claybits.xyz (see `CNAME` and `.github/workflows/pages.yml`). Done. Porting work happens in-session; pushes are run locally.
