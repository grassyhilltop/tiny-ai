...
## Driving a web prototype from voice mode (a pattern worth stealing)

This started as a bug report: the tutor's cursor worked in a text chat and went dead the moment
you pressed the voice button. The cause turns out to be general, so it is written up here rather
than buried in the lab.

**Voice mode does not get custom MCP connectors.** Not on Claude, and not on ChatGPT either.

- Claude voice mode has web search and a first-party connector allowlist (Gmail, Calendar, Docs,
  Slack, Canva, Notion). Custom MCP servers are not discovered in voice chat; the bug report
  asking for it was [closed as not planned](https://github.com/anthropics/claude-ai-mcp/issues/146),
  reproduced on both web and Android.
- ChatGPT is the same: MCP servers are not available as tools while Advanced Voice is running.
- Voice also is not in Claude Code or Cowork; those have dictation, not two-way voice.

So if your prototype's control channel is MCP, voice will never reach it, and no amount of
prompting fixes that. The way through is to stop needing MCP.

### The pattern: one URL per action, plain text back

The one general-purpose tool voice *does* have is fetching a web page. So expose every action as
an ordinary `GET` that returns plain text:

```
GET  RELAY/v/ROOM                                  look
GET  RELAY/v/ROOM/point/dose?say=what+does+this+do  act, and ask
GET  RELAY/v/ROOM/highlight/too+much+does+harm
GET  RELAY/v/ROOM/clear
```

`tutor-bridge/server.mjs` in this repo is a working 300-line example: the same dispatch behind
both an MCP door and a URL door, with a browser page attached over SSE.

Five things decide whether it works, all of them learned by getting them wrong:

1. **Answer in plain text, not JSON.** A fetching model reads a *web page*: whatever you return
   is flattened and skim-read. Braces and quotes get summarised away and the model starts
   inventing fields. Prose survives.
2. **Every response acts and then reports the state.** Voice round-trips are slow, and a model
   that needs one call to act and another to look will make one and guess at the other.
3. **Force cache-busting, loudly.** Fetched pages are cached hard. Tell the model to add a
   changing `?n=` to every call, in the response body itself, because it is the one that has to
   remember. Without it your agent narrates a screen from five minutes ago, confidently.
4. **Decode `+` in the path as a space.** In a path a `+` is a literal plus; models write
   `/highlight/too+much+does+harm` anyway because query strings taught them to. Accept it.
5. **Get the prompt in before voice starts.** You cannot paste into a voice session. The
   workflow is: paste the setup into a normal text chat, *then* press the voice button in that
   same conversation, so the URLs are already in context.

Side effects on a `GET` is normally a sin. It is the right call here, and the way to keep it
safe is to make the whole surface incapable of harm: this one can point, highlight and talk, and
cannot click, type, or change the user's work. The worst a leaked room code buys anyone is a
moving cursor.

### The same thing as a prompt you can paste

If you would rather hand this to your own coding agent than read the above:

```
Build me a voice-drivable control channel for my web prototype.

Context: voice mode (Claude and ChatGPT both) does not get custom MCP connectors. It gets web
search / page fetching. So MCP is a dead end for voice and the channel has to be plain URLs.

Build a tiny zero-dependency relay with three faces:
  1. my page attaches over SSE and receives commands
  2. an MCP endpoint for text chats (POST /mcp/:room)
  3. a URL endpoint for voice: GET /v/:room[/action[/arg]]  <- the point of the exercise

Rules for the /v/ surface, all of these matter:
- respond text/plain, never JSON. A fetcher flattens the page and skim-reads it; JSON gets
  mangled and the model starts inventing fields.
- every response does the action AND then prints the live page state, so one fetch per turn is
  enough. Lead with whatever answers "what is the user looking at" (pointer target, selection).
- end every response with a short menu of the available URLs, so the model can rediscover the
  API from any single fetch.
- in that menu, tell the model to add a different ?n=NUMBER to every call. Fetches are cached
  and stale state is worse than no state.
- decode "+" in path segments as a space. Models write /say/hello+there regardless of spec.
- Cache-Control: no-store, and CORS open.
- unknown action: do not error, print the menu.
- nobody connected: say so in a sentence a model can act on, not a 404.

Keep the action set incapable of harm (point / highlight / annotate / read), never click or
type or mutate the user's data, and say so in the response. GET with side effects is fine only
because of that.

Then give me the setup text a user pastes into a TEXT chat before pressing the voice button,
containing the literal URLs. You cannot paste into a voice session, so it has to be in context
first.
```
